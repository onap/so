...
public class MsoCloudifyUtils extends MsoCommonUtils implements VduPlugin {
    ...
    protected DeploymentV31 queryDeployment(String cloudSiteId, String deploymentId) throws MsoException {
        // Obtain the cloud site information where we will load the blueprint
        Optional<CloudSite> cloudSite = cloudConfig.getCloudSite(cloudSiteId);
        if (!cloudSite.isPresent()) {
            throw new MsoCloudSiteNotFound(cloudSiteId);
    ...
    protected DeploymentState getDeploymentStatus(String cloudSiteId, DeploymentV31 deployment) throws MsoException {
        // Obtain the cloud site information where we will load the blueprint
        Optional<CloudSite> cloudSite = cloudConfig.getCloudSite(cloudSiteId);
        if (!cloudSite.isPresent()) {
            throw new MsoCloudSiteNotFound(cloudSiteId);
     ...
     */
    public APIV31 getCloudifyClient(CloudSite cloudSite) throws MsoException {
