...
public class MsoMulticloudUtils extends MsoHeatUtils implements VduPlugin{
    ...
    public StackInfo createStack (String cloudSiteId,
		...
		MulticloudHeat heatClient = getMulticloudHeatClient (cloudSite, tenantId);
		if (heatClient != null) {
  ...
  //              		alarmLogger.sendAlarm(HEAT_ERROR, MsoAlarmLogger.CRITICAL, me.getContextMessage());
   //             		throw me;
    ...
    public StackInfo queryStack (String cloudSiteId, String tenantId, String stackName) throws MsoException {
		...
		// Get a Heat client. They are cached between calls (keyed by tenantId:cloudId)
		MulticloudHeat heatClient = null;
		try {
			heatClient = getMulticloudHeatClient(cloudSite, tenantId);
			if (heatClient != null) {
				LOGGER.debug("Found: " + heatClient.toString());
			}
	 ...
	 */
	public MulticloudHeat getMulticloudHeatClient (CloudSite cloudSite, String tenantId) throws MsoException {
		...
		if (heatClientCache.containsKey (cacheKey)) {
			if (!heatClientCache.get (cacheKey).isExpired ()) {
				...
				return heatClientCache.get (cacheKey).getMulticloudHeatClient();
			} else {
