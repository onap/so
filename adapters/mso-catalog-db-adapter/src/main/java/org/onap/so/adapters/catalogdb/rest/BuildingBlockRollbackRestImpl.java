...
public class BuildingBlockRollbackRestImpl {
    ...
    @GET
    @Operation(description = "Look up BuildingBlock Rollback List", responses = @ApiResponse(content = @Content(array = @ArraySchema(schema = @Schema(implementation = BuildingBlockRollback.class)))))
    @Produces({ MediaType.APPLICATION_JSON })
    @Transactional(readOnly = true)
