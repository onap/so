...
package org.openecomp.mso.adapters.catalogdb.rest;
...
import org.openecomp.mso.db.catalog.data.repository.ArRecipeRepository;
...
import org.openecomp.mso.db.catalog.data.repository.ToscaCsarRepository;
	...
	public Response resourceRecipe(@QueryParam("resourceModelUuid") String rmUuid, @QueryParam("action") String action) {
		...
		try {
			if (rmUuid != null && !"".equals(rmUuid)) {
				...
				Recipe recipe = vnfRecipeRepo.findVnfRecipeByVnfTypeAndAction(vnf.getModelName(), action);
				if (null == recipe) {
