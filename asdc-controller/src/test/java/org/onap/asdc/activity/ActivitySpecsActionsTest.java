...
public class ActivitySpecsActionsTest extends BaseTest{
    ...
    public void CreateActivitySpec_Test() throws Exception {    	
    	...
    	wireMockServer.stubFor(post(urlPathMatching("/v1.0/activity-spec"))
	               .willReturn(aResponse().withHeader("Content-Type", "application/json").withStatus(org.springframework.http.HttpStatus.OK.value()).withBody(body)));
    ...
    public void CertifyActivitySpec_Test() throws Exception {    	
    	...
    	wireMockServer.stubFor(put(urlPathMatching(urlPath))
	               .willReturn(aResponse().withHeader("Content-Type", "application/json").withStatus(org.springframework.http.HttpStatus.OK.value())));
