...
public class ToscaResourceInstallerTest extends BaseTest {
	...
	@Test
	@Transactional
	public void installTheResourceTestForBondingService() throws Exception {
		populateToscaResourceStruct();
		populatevfResourceStructure();
		populateNotificationData();
		toscaInstaller.installTheResource(toscaResourceStruct, vfResourceStructure);

		Service actualService = serviceRepo.findByServiceType(catalogService.getServiceType());
		ExternalServiceToInternalService actualInternalService = externalServiceToInternalServiceRepository
		.findByServiceNameAndSubscriptionServiceType("AVPNBondingToCollaborate",catalogService.getServiceRole());

		assertEquals(actualService.getModelUUID(),actualInternalService.getServiceModelUUID());
		assertThat(actualService, sameBeanAs(catalogService)
				.ignoring("created")
				.ignoring("networkCustomizations.0x1.created")
				.ignoring("networkCustomizations.0x1.networkResource.created")
				.ignoring("vnfCustomizations.0x1.created")
				.ignoring("vnfCustomizations.0x1.vnfResources.created")
				.ignoring("allottedCustomizations.0x1.created")
				.ignoring("allottedCustomizations.0x1.allottedResource.created"));
		verify(toscaResourceStruct, times(1)).setSuccessfulDeployment();
	}
	...
	@Test
	@Transactional
	public void installTheResourceTestForBondingServiceExists() throws Exception {
		populateToscaResourceStruct();
		populatevfResourceStructure();
		populateNotificationData();
		
		ExternalServiceToInternalService serviceToInternalService = new ExternalServiceToInternalService();
		serviceToInternalService.setServiceName("AVPNBondingToCollaborate");
		serviceToInternalService.setSubscriptionServiceType("COLLABORATE");
		serviceToInternalService.setServiceModelUUID("5df8b6de-2083-11e7-93ae-92361f002612");
	...
	@Test
	@Transactional
	public void installTheResourceTestServiceTypeNotBonding() throws Exception {
		populateToscaResourceStruct();
		populatevfResourceStructure();
		populateNotificationData();
		catalogService.setServiceType("serviceType");
		...
		assertNull(actualInternalService);
		assertThat(actualService, sameBeanAs(catalogService)
				.ignoring("created")
				.ignoring("networkCustomizations.0x1.created")
				.ignoring("networkCustomizations.0x1.networkResource.created")
	...
	@Test
	@Transactional
	public void installTheResourceTestNotAvpnServiceRole() throws Exception {
		populateToscaResourceStruct();
		populatevfResourceStructure();
		populateNotificationData();
		catalogService.setServiceType("serviceType");
		catalogService.setServiceRole("NOT_COLLABORATE");
