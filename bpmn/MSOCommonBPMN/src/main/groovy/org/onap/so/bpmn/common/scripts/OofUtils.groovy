...
class OofUtils {
    ...
    Void createCloudSiteCatalogDb(CloudSite cloudSite, DelegateExecution execution) {
        ...
        try {
            CatalogDbClient getCatalogDbClient = new CatalogDbClient()
            getCloudsite = getCatalogDbClient.getCloudSite(cloudSite.getId().toString())
        } catch (Exception e) {
            e = null
     ...
     */
    HomingInstance getHomingInstance(String serviceInstanceId) {

        CatalogDbClient catalogDbClient = new CatalogDbClient()
        try {
            return catalogDbClient.getHomingInstance(serviceInstanceId)
        } catch (Exception exception) {
            utils.log("DEBUG", "Could not get HomingInstance for serviceInstanceId : " + serviceInstanceId, true)
            utils.log("DEBUG", "Get HomingInstance Error: " + exception, true)
        }
