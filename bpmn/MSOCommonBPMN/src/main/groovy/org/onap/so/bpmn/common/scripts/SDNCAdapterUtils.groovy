...
class SDNCAdapterUtils {
	...
	private void SDNCAdapterActivateRequest(DelegateExecution execution, String resultVar, String svcAction,
			...
			if (callbackUrl == null || callbackUrl.trim() == "") {
				logger.error("{} {} {} {} {} {}", MessageEnum.BPMN_GENERAL_EXCEPTION_ARG.toString(), 'mso:workflow:sdncadapter:callback URN is not set', "BPMN", MsoLogger.getServiceName(), MsoLogger.ErrorCode.UnknownError.getValue(), "");
			...
			public void validateL3BondingSDNCResp(DelegateExecution execution, String response, WorkflowException workflowException, boolean success) {
				...
				try {
					...
					if (success) {
						...
						logger.warn(MessageEnum.BPMN_GENERAL_WARNING, 'sdncAdapter did not complete successfully, sdncAdapter Success Indicator was false ', "BPMN", MsoLogger.getServiceName(), MsoLogger.ErrorCode.UnknownError, 'sdncAdapter did not complete successfully, sdncAdapter Success Indicator was false ');
						msoLogger.warn(MessageEnum.BPMN_GENERAL_WARNING.toString(), 'sdncAdapter did not complete successfully, sdncAdapter Success Indicator was false ', "BPMN", MsoLogger.getServiceName(), MsoLogger.ErrorCode.UnknownError.getValue(), 'sdncAdapter did not complete successfully, sdncAdapter Success Indicator was false ');
					...
					if (response == null || response.trim().equals("")) {
						logger.warn(MessageEnum.BPMN_GENERAL_WARNING, 'sdncAdapter workflow response is empty', "BPMN", MsoLogger.getServiceName(), MsoLogger.ErrorCode.UnknownError, 'sdncAdapter workflow response is empty');;
						msoLogger.warn(MessageEnum.BPMN_GENERAL_WARNING.toString(), 'sdncAdapter workflow response is empty', "BPMN", MsoLogger.getServiceName(), MsoLogger.ErrorCode.UnknownError.getValue(), 'sdncAdapter workflow response is empty');;
