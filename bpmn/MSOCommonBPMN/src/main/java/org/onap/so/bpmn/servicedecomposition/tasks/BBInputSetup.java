...
public class BBInputSetup implements JavaDelegate {
	 ...
	 *
	 * It will get the BB from the execution object by checking if the aLaCarte and
	 * homing is true.
	 *
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		try {
			...
			logger.debug("Exception occurred in BBInputSetup execute");
	...
	protected void populateVolumeGroup(ModelInfo modelInfo, Service service, String bbName,
		...
		if (vnf != null && vnfModelCustomizationUUID != null) {
			...
			if (volumeGroup == null && bbName.equalsIgnoreCase(AssignFlows.VOLUME_GROUP.toString())) {
				volumeGroup = createVolumeGroup(lookupKeyMap, resourceId, instanceName, generatedVnfType,
						instanceParams);
	 ...
	 *
	 * This will map the serviceInstanceAAI to serviceInstance and return the
	 * serviceInstance.
	 *
	 * @throws Exception
	 * @return serviceInstance
	...
	protected void mapCollection(List<AAIResultWrapper> collections, ServiceInstance serviceInstance) {
		if (!collections.isEmpty()) {
			...
			List<InstanceGroup> instanceGroupsList = mapInstanceGroups(
					relationships.getByType(AAIObjectType.INSTANCE_GROUP));
