        ...
        if (vnf != null && vnfModelCustomizationUUID != null) {
            for (VolumeGroup volumeGroupTemp : vnf.getVolumeGroups()) {
                if (parameter.getLookupKeyMap().get(ResourceKey.VOLUME_GROUP_ID) != null
                    ...
                    if (volumeGroupTemp.getModelInfoVfModule() == null) {
                        throw new Exception(
                                "ModelInfoVfModule is null for VolumeGroup: " + volumeGroupTemp.getVolumeGroupId());
                    }
                    String volumeGroupCustId = volumeGroupTemp.getModelInfoVfModule().getModelCustomizationUUID();
