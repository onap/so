...
public class BBInputSetup implements JavaDelegate {
    ...
    protected void addRelationshipsToSI(org.onap.aai.domain.yang.ServiceInstance serviceInstanceAAI,
        ...
        if (relationshipsOp.isPresent()) {
            mapRelationship(serviceInstance, relationshipsOp);
        }
    }

    private void mapRelationship(ServiceInstance serviceInstance, Optional<Relationships> relationshipsOp) {
    ...
    protected void mapCollection(List<AAIResultWrapper> collections, ServiceInstance serviceInstance) {
        if (!collections.isEmpty()) {
            ...
            if (aaiCollectionOp.isPresent()) {
                serviceInstanceSetCollection(serviceInstance, collectionWrapper, aaiCollectionOp);
            }
        }
    }
    ...
    private void serviceInstanceSetCollection(ServiceInstance serviceInstance, AAIResultWrapper collectionWrapper,
            Optional<org.onap.aai.domain.yang.Collection> aaiCollectionOp) {
        Collection collection = getCollection(aaiCollectionOp);
        Optional<Relationships> relationshipsOp = collectionWrapper.getRelationships();
        if (relationshipsOp.isPresent()) {
            getInstanceGroup(collection, relationshipsOp);
        } else {
    ...
    private void getInstanceGroup(Collection collection, Optional<Relationships> relationshipsOp) {
        Relationships relationships;
        List<InstanceGroup> instanceGroupsList;
        relationships = relationshipsOp.get();
        instanceGroupsList = mapInstanceGroups(relationships.getByType(AAIObjectType.INSTANCE_GROUP));
        if (!instanceGroupsList.isEmpty()) {
            collection.setInstanceGroup(instanceGroupsList.get(0));
        }
    }

    private Collection getCollection(Optional<org.onap.aai.domain.yang.Collection> aaiCollectionOp) {
    ...
    protected void mapProject(List<AAIResultWrapper> projects, ServiceInstance serviceInstance) {
        if (!projects.isEmpty()) {
            ...
            if (aaiProjectOp.isPresent()) {
                org.onap.aai.domain.yang.Project aaiProject;
