...
public class AbstractCDSProcessingBBUtils {
	...
	public void preProcessRequest(DelegateExecution execution) {
		...
		try {
			...
			CommonHeader commonHeader = CommonHeader.newBuilder().setOriginatorId(originatorId).setRequestId(requestId).setSubRequestId(subRequestId).build();
			ActionIdentifiers actionIdentifiers = ActionIdentifiers.newBuilder().setBlueprintName(blueprintName).setBlueprintVersion(blueprintVersion).setActionName(actionName).setMode(mode).build();

			Builder struct = Struct.newBuilder();
			try {
				JsonFormat.parser().merge(payload, struct);
			} catch (InvalidProtocolBufferException e) {
				logger.error("Failed to parse received message. blueprint({}:{}) for action({}). {}", blueprintVersion, blueprintName, actionName, e);
			}

			ExecutionServiceInput executionServiceInput = ExecutionServiceInput.newBuilder().setCommonHeader(commonHeader).setActionIdentifiers(actionIdentifiers).setPayload(struct).build();
	...
	public void sendRequestToCDSClient(DelegateExecution execution) {
		...
		try {
			...
			ExecutionServiceInput executionServiceInput = (ExecutionServiceInput) execution.getVariable("executionServiceInput");
	...
	public String constructPayload(Map<String, Object> parameters, String nfType, String configType) {
		...
		JSONObject entityParameters = new JSONObject();
		...
		if (null != nfType && nfType.equalsIgnoreCase("VNF")) {
			if (null != parameters && null != parameters.get("vnf-id") && null != parameters.get("vnf-name") && null != parameters.get("vnf-customization-uuid")) {
				...
				entityParameters.put("vnf_id", vnfId);
				entityParameters.put("vnf-name", vnfName);
				entityParameters.put("vnf-customization-uuid", vnfCustomizationUuid);
			}
		}
		if (nfType != null && nfType.equalsIgnoreCase("PNF")) {
			if (null != parameters && null != parameters.get("pnf-id") && null != parameters.get("pnf-name") && null != parameters.get("pnf-customization-uuid")) {
				...
				entityParameters.put("pnf_id", pnfId);
				entityParameters.put("pnf-name", pnfName);
				entityParameters.put("pnf-customization-uuid", pnfCustomizationUuid);
			}
		}
		if (null != parameters && null != parameters.get("service-instance-id")) {
			...
			entityParameters.put("service-instance-id", serviceIntanceId);
		}
		if (null != parameters && null != parameters.get("service-model-uuid")) {
			...
			entityParameters.put("service-model-uuid", serviceModelUuid);
		}

		if ((null != parameters && null != parameters.get("user_params")) && configType.equalsIgnoreCase("ConfigAssign")) {
		...
		configPropertiesParameters.put("entity", entityParameters);
