...
public class CDSProcessingListenerImpl implements CDSProcessingListener {
	...
	public void onMessage(ExecutionServiceOutput message) {
		...
		if (message.getStatus().getEventType().equals("EVENT-COMPONENT-FAILURE")) {
			// failed processing with failure
			// Update RequestDB Failed Status
		} else if (message.getStatus().getEventType().equals("EVENT-COMPONENT-PROCESSING")) {
			// still processing
			// Update RequestDB Status InProgress
		} else if (message.getStatus().getEventType().equals("EVENT_COMPONENT_EXECUTED")) {
