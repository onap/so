...
public class GeneratePayloadForVnf implements NetworkFunction {
    ...
    public Optional<String> buildRequestPayload(String action) throws Exception {
        ...
        String requestBuilder =
                "{\"" + action + "-" + VNF_SCOPE + "-request\":{" + "\"resolution-key\":" + "\"" + resolutionKey + "\","
    ...
    private JsonObject buildPropertyObjectForVnf() throws Exception {
        ...
        // At this moment when we do config-deploy we don’t provide instance parameters. The instance parameters are
        // added
        // when we do config-assign.
        if (action.equals("config-assign")) {
