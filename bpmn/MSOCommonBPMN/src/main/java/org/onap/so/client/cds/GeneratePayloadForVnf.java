...
public class GeneratePayloadForVnf implements NetworkFunction {
    ...
    GeneratePayloadForVnf(ExtractPojosForBB extractPojosForBB) {
    ...
    public Optional<String> buildRequestPayload(String action) throws Exception {
        ...
        String requestBuilder = "{\"" + action + "-" + "request\":{" + "\"resolution-key\":" + "\"" + resolutionKey
                + "\"," + PropertyPayloadBuilder.buildConfigProperties(action, vnfObject) + '}';
    ...
    private JsonObject buildPropertyObjectForVnf() throws Exception {
        ...
        // At this moment when we do configDeploy we don’t provide instance parameters. The instance parameters are
        // added when we do configAssign.
        if (action.equals("configAssign")) {
