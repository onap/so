...
public class VnfTopologyOperationRequestMapper {
	...
	public GenericResourceApiVnfOperationInformation reqMapper(SDNCSvcOperation svcOperation,
			SDNCSvcAction svcAction,GenericResourceApiRequestActionEnumeration requestAction, GenericVnf vnf, ServiceInstance serviceInstance,
		...
		GenericResourceApiRequestinformationRequestInformation requestInformation = generalTopologyObjectMapper.buildGenericResourceApiRequestinformationRequestInformation(sdncReqId, requestAction);
		...
		if(homing){
			String licenseKeyGroupUuid = vnf.getLicense().getLicenseKeyGroupUuids().get(0);
			String entitlementPoolUuid = vnf.getLicense().getEntitlementPoolUuids().get(0);
			GenericResourceApiParamParam paramItem = new GenericResourceApiParamParam();
		...
		for(InstanceGroup instanceGroup : instanceGroups) {
			if(ModelInfoInstanceGroup.TYPE_NETWORK_INSTANCE_GROUP.equalsIgnoreCase(instanceGroup.getModelInfoInstanceGroup().getType())) {
