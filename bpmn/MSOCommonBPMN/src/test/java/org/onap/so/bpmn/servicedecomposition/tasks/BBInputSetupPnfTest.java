...
public class BBInputSetupPnfTest {
    ...
    public void populatePnfShouldSetRequiredFields() {
        ...
        Pnf pnf = BBInputSetupPnf.populatePnf(pnfs, pnfId, serviceInstance);

        assertEquals(pnf.getPnfId(), pnfId);
        assertEquals(pnf.getPnfName(), pnfName);
        assertEquals(pnf.getModelInfoPnf().getModelCustomizationUuid(), modelCustomizationId);
        assertEquals(pnf.getModelInfoPnf().getModelInvariantUuid(), modelInvariantId);
        assertEquals(pnf.getModelInfoPnf().getModelUuid(), modelVersionId);
        assertEquals(pnf.getOrchestrationStatus(), OrchestrationStatus.PRECREATED);
        assertEquals(serviceInstance.getPnfs().get(0), pnf);
    }
}
