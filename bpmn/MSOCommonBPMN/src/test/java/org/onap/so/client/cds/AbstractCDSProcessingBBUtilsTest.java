...
public class AbstractCDSProcessingBBUtilsTest {
	...
	public void preProcessRequestTest() throws Exception {
		...
		abstractCDSProcessingBBUtils.preProcessRequest(execution);
		assertTrue(true);
	}

	@Test
	public void sendRequestToCDSClientTest() throws Exception {
		ExecutionServiceInput fakeRequest = ExecutionServiceInput.newBuilder().setActionIdentifiers(
	            ActionIdentifiers.newBuilder().setActionName("actionName").build()).build();
		
		DelegateExecution execution = mock(DelegateExecution.class);
		when(execution.getVariable("executionServiceInput")).thenReturn(fakeRequest);
	...
	@Test
	public void sendRequestToCDSClientTestFail() throws Exception {
		ExecutionServiceInput fakeRequest = ExecutionServiceInput.newBuilder().setActionIdentifiers(
	            ActionIdentifiers.newBuilder().setActionName("actionName").build()).build();
