...
public class GeneratePayloadForCdsTest {
    ...
    private static final String VF_MODULE_SCOPE = "vf-module";
    ...
    public void testRequestPayloadForConfigAssignVnf() throws Exception {
        ...
        buildingBlock.setBpmnAction(ASSIGN_ACTION);
        buildingBlock.setBpmnScope(VNF_SCOPE);

        executeBuildingBlock.setBuildingBlock(buildingBlock);

        buildingBlockExecution.setVariable("buildingBlock", executeBuildingBlock);

        GeneratePayloadForCds configurePayloadForCds = new GeneratePayloadForCds(buildingBlockExecution, extractPojosForBB);
        ...
        when(extractPojosForBB.extractByKey(buildingBlockExecution, ResourceKey.SERVICE_INSTANCE_ID))
                .thenReturn(instance);
        when(extractPojosForBB.extractByKey(buildingBlockExecution, ResourceKey.GENERIC_VNF_ID)).thenReturn(createGenericVnf());

        // when
        String payload = String.valueOf(configurePayloadForCds.buildPayloadForCds());
    ...
    public void testRequestPayloadForConfigAssignVfModule() throws Exception {
        ...
        BuildingBlock buildingBlock = new BuildingBlock();
        buildingBlock.setBpmnAction(DEPLOY_ACTION);
        buildingBlock.setBpmnScope(VF_MODULE_SCOPE);
        ...
        when(extractPojosForBB.extractByKey(buildingBlockExecution, ResourceKey.SERVICE_INSTANCE_ID))
                .thenReturn(instance);
        when(extractPojosForBB.extractByKey(buildingBlockExecution, ResourceKey.GENERIC_VNF_ID)).thenReturn(createGenericVnf());
        when(extractPojosForBB.extractByKey(buildingBlockExecution, ResourceKey.VF_MODULE_ID)).thenReturn(createVfModule());
