...
public class MessageSendingHandlerRetryTest {
    ...
    public void testCorrelateRetriesOnDeadlock() throws Exception {
        ...
        SQLTransactionRollbackException deadlock =
                new SQLTransactionRollbackException("Deadlock found when trying to get lock; try restarting transaction");
        TransactionException txException = new TransactionException("Unable to commit against JDBC Connection", deadlock);
        JpaSystemException jpaException = new JpaSystemException(txException);

        doThrow(jpaException)
                .doReturn(mock(MessageCorrelationResult.class))
                .when(correlationBuilder).correlate();
    ...
    private CDSProcessingListener createMessageSendingHandler(MessageCorrelationBuilder correlationBuilder,
        ...
        Class<?> handlerClass = Class.forName(
                "org.onap.so.client.cds.AbstractCDSProcessingBBUtils$MessageSendingHandler");
        Constructor<?> constructor = handlerClass.getDeclaredConstructor(
                AbstractCDSProcessingBBUtils.class, MessageCorrelationBuilder.class);
    ...
    private ExecutionServiceOutput buildSuccessResponse() {
        return ExecutionServiceOutput.newBuilder()
                .setCommonHeader(CommonHeader.newBuilder().setRequestId("test-request").setSubRequestId("test-sub").build())
