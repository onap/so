...
public class MessageSendingHandlerRetryTest {
    ...
    public void testCorrelateRetriesOnDeadlock() throws Exception {
        ...
        TransactionException txException =
                new TransactionException("Unable to commit against JDBC Connection", deadlock);
        JpaSystemException jpaException = new JpaSystemException(txException);

        doThrow(jpaException).doNothing().when(correlationBuilder).correlate();
    ...
    public void testCorrelateSucceedsWithoutDeadlock() throws Exception {
        ...
        // correlate() is void — default mock behavior is do nothing (success)
    ...
    private CDSProcessingListener createMessageSendingHandler(MessageCorrelationBuilder correlationBuilder,
        ...
        Class<?> handlerClass =
                Class.forName("org.onap.so.client.cds.AbstractCDSProcessingBBUtils$MessageSendingHandler");
