...
@Ignore
public class SDNCAdapterV1IT extends BaseIntegrationTest {
	...
	MsoLogger logger = MsoLogger.getMsoLogger(MsoLogger.Catalog.BPEL,SDNCAdapterV1IT.class);
	

	public SDNCAdapterV1IT() throws IOException {
	...
	private WorkflowResponse invokeFlow(String workflowRequest) {
		...
		Response response = workflowResourceSync.startProcessInstanceByKey("sdncAdapter", varMap);
	...
	public void sunnyDay() throws InterruptedException {
		...
		logger.debug("Injecting SDNC Adapter asynchronous callback message to continue processing");
		String generatedRequestId = (String) processEngine.getRuntimeService().getVariable(pid, "SDNCA_requestId");
	...
	public void nonFinalWithTimeout() throws InterruptedException {
		...
		//logger.debug("SDNCAdapter interim status processing flow Started!");
		...
		//logger.debug("SDNCAdapter interim status processing flow Completed!");
	}
	...
	public void nonFinalThenFinal() throws InterruptedException {
		...
		//logger.debug("SDNCAdapter non-final then final processing flow Started!");
		...
		//logger.debug("SDNCAdapter non-final then final processing flow Completed!");
	}
	...
	public void nonFinalThenFinalWithNotify() throws InterruptedException {
		...
		try {
			...
			logger.debug("modified request: " + modSdncAdapterWorkflowRequestAct);
		} catch (Exception e) {
			logger.debug("request modification failed");
			//e.printStackTrace();
		}
	...
	private void waitForExecutionToStart(String processDefintion, int count) throws InterruptedException {
		//logger.debug(processEngine.getRuntimeService().createExecutionQuery().processDefinitionKey(processDefintion).count());
		while (processEngine.getRuntimeService().createExecutionQuery().processDefinitionKey(processDefintion).count() != count) {
			Thread.sleep(200);
		}
	}

	@Test
	 // Ignored because PropertyConfigurationSetup is timing out
	
	public void badCorrelationIdTest() throws InterruptedException, IOException {

		mockSDNCAdapter(200);

		//logger.debug("SDNCAdapter bad RequestId test Started!");
		...
		//logger.debug("SDNCAdapter bad RequestId test Completed!");
	}
	...
	public void badSynchronousResponse() throws IOException, InterruptedException {
		...
		//logger.debug("SDNCAdapter bad synchronous response flow Started!");
...
//		assertProcessInstanceFinished(response.getProcessInstanceID());
		//logger.debug("SDNCAdapter bad synchronous response flow Completed!");
	}
	...
	public void sdncNotFound() throws IOException, InterruptedException {
		...
		//logger.debug("SDNCAdapter SDNC Notfound test Completed!");
	}
	...
	public void asynchronousMessageTimeout() throws IOException, InterruptedException {
		mockSDNCAdapter(200);
		//logger.debug("SDNCAdapter asynchronous message timeout flow Started!");
		ProcessExecutionThread thread = new ProcessExecutionThread(sdncAdapterWorkflowRequest);
		thread.start();
		waitForExecutionToStart("sdncAdapter", 3);
	...
	private void checkForTimeout(String pid) throws InterruptedException {
	    ...
	    Job timerJob = processEngine.getManagementService().createJobQuery().processInstanceId(pid).singleResult();
		...
		//logger.debug("SDNCAdapter asynchronous message timeout flow Completed!");
	}
	...
	private String getPid() {
		return processEngine.getHistoryService().createHistoricProcessInstanceQuery().list().get(0).getId();
	}
	...
	private void assertProcessInstanceFinished(String pid) {
	    assertEquals(1, processEngine.getHistoryService().createHistoricProcessInstanceQuery().processInstanceId(pid).finished().count());
	}

	private void assertProcessInstanceNotFinished(String pid) {
	    assertEquals(0, processEngine.getHistoryService().createHistoricProcessInstanceQuery().processInstanceId(pid).finished().count());
	}
