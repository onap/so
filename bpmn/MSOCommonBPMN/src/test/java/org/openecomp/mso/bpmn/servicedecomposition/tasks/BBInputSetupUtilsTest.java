...
public class BBInputSetupUtilsTest {
	...
	public void testGetCloudRegion() {
		...
		doReturn(expected).when(MOCK_aaiResourcesClient).get(org.onap.aai.domain.yang.CloudRegion.class,
				AAIUriFactory.createResourceUri(AAIObjectType.CLOUD_REGION, cloudOwner,
						requestDetails.getCloudConfiguration().getLcpCloudRegionId()));
		org.onap.aai.domain.yang.CloudRegion actual = this.bbInputSetupUtils.getCloudRegion(requestDetails,
				cloudOwner);
		assertThat(actual, sameBeanAs(expected.get()));
	}
	...
	public void testGetCloudRegionExceptionTest() {
		...
		CloudRegion cloudRegion = bbInputSetupUtils.getCloudRegion(requestDetails, cloudOwner);
	...
	@Test
	public void testGetOptionalAAIServiceInstanceByName() throws Exception {
		String globalCustomerId = "globalCustomerId";
		String serviceType = "serviceType";
		String serviceInstanceId = "serviceInstanceId";
		...
		ServiceInstance expected = new ServiceInstance();
		expected.setServiceInstanceId("serviceInstanceId");
		expected.setServiceType(serviceType);
		expected.setServiceInstanceName("serviceInstanceName");

		ServiceInstances serviceInstances = new ServiceInstances();
		serviceInstances.getServiceInstance().add(expected);
	...
	public void testGetCatalogInstanceGroup() throws JsonParseException, JsonMappingException, IOException {
		...
		org.openecomp.mso.db.catalog.beans.InstanceGroup expectedInstanceGroup = mapper.readValue(
				new File(RESOURCE_PATH + "InstanceGroup.json"), org.openecomp.mso.db.catalog.beans.InstanceGroup.class);
		
		doReturn(expectedInstanceGroup).when(MOCK_catalogDbClient).getInstanceGroupByModelUUID(isA(String.class));
		
		org.openecomp.mso.db.catalog.beans.InstanceGroup actualInstanceGroup = bbInputSetupUtils.getCatalogInstanceGroup(modelUUID);
	...
	@Test
	public void testGetAAIGenericVnfWrapper() {
		AAIResourceUri aaiResourceUri = AAIUriFactory.createResourceUri(AAIObjectType.GENERIC_VNF);
		
		AAIResultWrapper expectedAaiResultWrapper = new AAIResultWrapper("aaiResultWrapperJson");

		doReturn(expectedAaiResultWrapper).when(MOCK_aaiResourcesClient).get(isA(AAIResourceUri.class));
		
		AAIResultWrapper actualAaiResultWrapper = bbInputSetupUtils.getAAIGenericVnf(aaiResourceUri);
		
		assertThat(actualAaiResultWrapper, sameBeanAs(expectedAaiResultWrapper));
	}
	
	@Test
	public void testGetAAIL3Network() {
		AAIResourceUri aaiResourceUri = AAIUriFactory.createResourceUri(AAIObjectType.L3_NETWORK);
		...
		AAIResultWrapper actualAaiResultWrapper = bbInputSetupUtils.getAAIL3Network(aaiResourceUri);
