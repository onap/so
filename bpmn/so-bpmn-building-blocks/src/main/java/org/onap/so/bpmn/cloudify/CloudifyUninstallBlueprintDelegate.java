...
/**
 * The purpose of this delegate is to provide the capability to "uninstall" (in the general sense)
 * a Cloudify blueprint.  This can mean just running the uninstall workflow, optionally deleting the deployment,
 * and optionally deleting the blueprint.
 * @author dewayne
 *
 */
public class CloudifyUninstallBlueprintDelegate extends AbstractJavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		Map<String,String> parms = execution.hasVariable(INP_UNINSTALL_PARMS_KEY)?(Map<String,String>)execution.getVariable(INP_UNINSTALL_PARMS_KEY):null;
		boolean deleteDeployment = execution.hasVariable(INP_DELETE_DEPLOYMENT_KEY)?Boolean.valueOf((String)execution.getVariable(INP_DELETE_DEPLOYMENT_KEY)):false;
		boolean deleteBlueprint = execution.hasVariable(INP_DELETE_BLUEPRINT_KEY)?Boolean.valueOf((String)execution.getVariable(INP_DELETE_BLUEPRINT_KEY)):false;
