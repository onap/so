...
public class InstallDelegateTest {
	...
	public void TestInstall() throws Exception{
		...
		when(execution.getVariable("InputCfy_blueprint")).thenReturn("InputCfy_blueprint", "tosca_definitions_version: cloudify_dsl_1_3\n" + 
				"imports:\n" + 
		...
		stubFor(put(urlMatching("/.*blueprints.*")).willReturn(aResponse().withBody("{\"id\":\"test\"}").withHeader("content-type","application/json").withStatus(200)));
		stubFor(get(urlMatching("/.*executions.*")).willReturn(aResponse().withBody("{\"items\":[{\"id\": \"1\",\"blueprint_id\": \"test\",\"deployment_id\": \"test\",\"created_at\": \"2009-01-01\", \"status\": \"terminated\"}]}").withHeader("content-type", "application/json").withStatus(200)));
		stubFor(get(urlMatching("/.*executions/[a-zA-Z0-9]*")).willReturn(aResponse().withBody("{\"id\": \"1\",\"blueprint_id\": \"test\",\"deployment_id\": \"test\",\"created_at\": \"2009-01-01\", \"status\": \"terminated\"}").withHeader("content-type", "application/json").withStatus(200)));
		stubFor(post(urlMatching("/.*executions.*")).willReturn(aResponse().withBody("{\"id\": \"1\",\"blueprint_id\": \"test\",\"deployment_id\": \"test\",\"created_at\": \"2009-01-01\", \"status\": \"terminated\"}").withHeader("content-type", "application/json").withStatus(200)));
		Thread.sleep(5000L);
		d.execute(execution);
	}
