...
public class UnInstallDelegateTest {
	...
	public void TestUninstallOnly() throws Exception{
		...
		stubFor(delete(urlMatching("/.*deployments/.*")).willReturn(aResponse().withStatus(404))); // fail if deployment delete
		stubFor(delete(urlMatching("/.*blueprints/.*")).willReturn(aResponse().withStatus(404))); // fail if blueprint delete
		d.execute(execution);
	}
