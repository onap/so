...
class DoDeallocateCoreNSSI extends DoCommonCoreNSSI {
    ...
    void preProcessRequest(DelegateExecution execution) {
        ...
        String terminateNSSI = jsonUtil.getJsonValue(execution.getVariable("sliceParams"), "isTerminateNSSI")
        if (isBlank(terminateNSSI)) {
            String msg = "isTerminateNSSI is mandatory parameter"
            exceptionUtil.buildAndThrowWorkflowException(execution, 500, msg)
        }
        else {
            boolean bTerminateNSSI = Boolean.parseBoolean(terminateNSSI)
            execution.setVariable("isTerminateNSSI", Boolean.toString(bTerminateNSSI))
        }
