...
class TnAllocateNssi extends AbstractServiceTaskProcessor {
    ...
    void prepareOofSelection(DelegateExecution execution) {
        ...
        Map<String, Object> profileInfo = objectMapper.readValue(execution.getVariable("tnNfSliceProfile"), Map.class)
        String modelUuid = execution.getVariable("modelUuid")
        String modelInvariantUuid = execution.getVariable("modelInvariantUuid")
    ...
    void processOofSelection(DelegateExecution execution) {
	    ...
	    JsonParser parser = new JsonParser()
            JsonArray solution = parser.parse(solutions as String)
            if(solution.size()>=1) {
    ...
    def createModifyNssiQueryJobStatus = { DelegateExecution execution ->
        ...
        serviceInfo.addProperty("nssiId", execution.getVariable("TnServiceInstanceId"))
