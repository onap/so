...
public class GenericRequestObjectCreater implements JavaDelegate {
    ...
    public enum Action {
        ...
        private final String name;
        private String yamlFile; //this yaml file name is complete when scope name is prepend with it e.g. pnf/sw-download.yaml or vnf/sw-download.yaml
        private Mode mode;
    ...
    public String getdynamicRequestObject(final DelegateExecution delegateExecution, Action action, Scope scope) {
        ...
        Yaml myYaml = new Yaml();
        InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream(getYamlFileNameUsingScopeAndAction(action, scope));
    ...
    /**
     * This method extracts action from execution object, if no action is found,
     * it will throw a (@link WorflowException).
     **/
    ...
    /**
     * This method extracts scope from execution object, if no scope is found default scope set
     * to (@link Scope.PNF).
     **/
