...
public class AAIQueryTasks {
	...
	public void queryNetworkVpnBinding(BuildingBlockExecution execution) {
		try {
			...
			List<org.openecomp.mso.bpmn.servicedecomposition.bbobjects.VpnBinding> mappedVpnBindings = new ArrayList<>();
			if(netBindingsUriList != null && !netBindingsUriList.isEmpty()) {
				for(AAIResourceUri netBindingUri : netBindingsUriList) {
					...
					if(oVpnBinding.isPresent()) {
						org.openecomp.mso.bpmn.servicedecomposition.bbobjects.VpnBinding mappedVpnBinding = 
								modelMapper.map(oVpnBinding.get(), org.openecomp.mso.bpmn.servicedecomposition.bbobjects.VpnBinding.class);
	...
	public void getNetworkVpnBinding(BuildingBlockExecution execution) {
		...
		try {
			...
			List<org.openecomp.mso.openstack.beans.RouteTarget> routeTargets = new ArrayList<>();
			for (AAIResourceUri netBindingUri : netBindingsUriList) {
				...
				if (oVpnBinding.isPresent()) {
					...
					if (rts != null) {
						...
						if (!rtList.isEmpty()) {
							PropertyMap<org.onap.aai.domain.yang.RouteTarget, org.openecomp.mso.openstack.beans.RouteTarget> personMap = new PropertyMap<org.onap.aai.domain.yang.RouteTarget, org.openecomp.mso.openstack.beans.RouteTarget>() {
								@Override
							...
							for (org.onap.aai.domain.yang.RouteTarget rt : rtList) {
								org.openecomp.mso.openstack.beans.RouteTarget openstackRtBean = modelMapper.map(rt,
										org.openecomp.mso.openstack.beans.RouteTarget.class);
			...
			if(!netPoliciesUriList.isEmpty()) {
				for(AAIResourceUri netPolicyUri : netPoliciesUriList) {
					...
					if(oNetPolicy.isPresent()) {
						l3Network.getNetworkPolicies().add(modelMapper.map(oNetPolicy.get(), org.openecomp.mso.bpmn.servicedecomposition.bbobjects.NetworkPolicy.class));
					}
				}
			}
		} catch(Exception ex) {
			...
			if(!routeTableUriList.isEmpty()) {
				for(AAIResourceUri routeTableUri : routeTableUriList) {
					...
					if(oRouteTableReference.isPresent()) {
						org.openecomp.mso.bpmn.servicedecomposition.bbobjects.RouteTableReference mappedRouteTableReference = modelMapper.map(oRouteTableReference.get(), org.openecomp.mso.bpmn.servicedecomposition.bbobjects.RouteTableReference.class);
	...
	private List<org.openecomp.mso.bpmn.servicedecomposition.bbobjects.RouteTarget> mapRouteTargets(List<org.onap.aai.domain.yang.RouteTarget> routeTargets) {
		List<org.openecomp.mso.bpmn.servicedecomposition.bbobjects.RouteTarget> mappedRouteTargets = new ArrayList<>();
		if(!routeTargets.isEmpty()) {
			for(org.onap.aai.domain.yang.RouteTarget routeTarget : routeTargets) {
				org.openecomp.mso.bpmn.servicedecomposition.bbobjects.RouteTarget mappedRouteTarget = modelMapper.map(routeTarget, org.openecomp.mso.bpmn.servicedecomposition.bbobjects.RouteTarget.class);
