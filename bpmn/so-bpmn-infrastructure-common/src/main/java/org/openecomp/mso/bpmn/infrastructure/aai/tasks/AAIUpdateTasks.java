...
public class AAIUpdateTasks {
	...
	public void updateOrchestrationStatusAssignedVfModule(BuildingBlockExecution execution) {
		try {
			...
			aaiVfModuleResources.updateOrchestrationStatusVfModule(vfModule,OrchestrationStatus.ASSIGNED);
		} catch (Exception ex) {			
			exceptionUtil.buildAndThrowWorkflowException(execution, 7000, ex);
		}
	}
	...
	public void updateOrchestrationStatusPendingActivationVfModule(BuildingBlockExecution execution) {
		try {
			VfModule vfModule = extractPojosForBB.extractByKey(execution, ResourceKey.VF_MODULE_ID, execution.getLookupMap().get(ResourceKey.VF_MODULE_ID));
			aaiVfModuleResources.updateOrchestrationStatusVfModule(vfModule,OrchestrationStatus.PENDING_ACTIVATION);
		} catch (Exception ex) {
			exceptionUtil.buildAndThrowWorkflowException(execution, 7000, ex);
		}
	}
	...
	public void updateOrchestrationStatusCreatedVfModule(BuildingBlockExecution execution) {		
		try {
			VfModule vfModule = extractPojosForBB.extractByKey(execution, ResourceKey.VF_MODULE_ID, execution.getLookupMap().get(ResourceKey.VF_MODULE_ID));
			aaiVfModuleResources.updateOrchestrationStatusVfModule(vfModule,OrchestrationStatus.CREATED);			
		} catch (Exception ex) {
			exceptionUtil.buildAndThrowWorkflowException(execution, 7000, ex);
		}
	}	
	...
	public void updateOrchestrationStatusActivateVfModule(BuildingBlockExecution execution) {
		...
		try {
			VfModule vfModule = extractPojosForBB.extractByKey(execution, ResourceKey.VF_MODULE_ID, execution.getLookupMap().get(ResourceKey.VF_MODULE_ID));
			aaiVfModuleResources.updateOrchestrationStatusVfModule(vfModule, OrchestrationStatus.ACTIVE);
