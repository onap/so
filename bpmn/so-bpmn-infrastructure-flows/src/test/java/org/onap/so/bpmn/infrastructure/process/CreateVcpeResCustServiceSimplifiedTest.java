...
public class CreateVcpeResCustServiceSimplifiedTest extends BaseBPMNTest {
    ...
    private static final int TIMEOUT_IN_MS = TIMEOUT_IN_MINUTES * 60 * 1000;
    ...
    public void setUp() {
        ...
        String requestObject = FileUtil.readResourceFile("request/" + getClass().getSimpleName() + ".json");
    ...
    public void workflow_validInput_expectedOuput() throws Exception {
        ...
        waitUntil(() -> isProcessInstanceEnded(pi));
    ...
    private void waitUntil(Supplier<Boolean> booleanSupplier) throws InterruptedException {
        while (!booleanSupplier.get()) {
            Thread.sleep(1000);
        }
    }

    private boolean isProcessInstanceEnded(ProcessInstance processInstance) {
        return runtimeService.createProcessInstanceQuery().processInstanceBusinessKey(processInstance.getBusinessKey())
                .list().isEmpty();
    }
    ...
    private void checkConfigDeploy(ExecutionServiceInput executionServiceInput) {
        ...
        Struct payload = executionServiceInput.getPayload();
        Struct request = payload.getFieldsOrThrow("config-deploy-request").getStructValue();

        assertThat(request.getFieldsOrThrow("resolution-key").getStringValue()).isEqualTo("PNFDemo");
        Struct properties = request.getFieldsOrThrow("config-deploy-properties").getStructValue();

        assertThat(properties.getFieldsOrThrow("pnf-name").getStringValue()).isEqualTo("PNFDemo");
        assertThat(properties.getFieldsOrThrow("service-model-uuid").getStringValue())
                .isEqualTo("f2daaac6-5017-4e1e-96c8-6a27dfbe1421");
        assertThat(properties.getFieldsOrThrow("pnf-customization-uuid").getStringValue())
                .isEqualTo("68dc9a92-214c-11e7-93ae-92361f002680");

        /*
         * IP addresses match the OAM ip addresses from AAI.
         */
        assertThat(properties.getFieldsOrThrow("pnf-ipv4-address").getStringValue()).isEqualTo("1.1.1.1");
        assertThat(properties.getFieldsOrThrow("pnf-ipv6-address").getStringValue()).isEqualTo("::/128");
    }
