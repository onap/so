...
import org.onap.so.client.oof.beans.HomingDemand;
...
import static org.onap.so.client.oof.OofValidator.validateSolution;
...
public class OofHomingV2 {
    ...
    private static final MsoLogger log = MsoLogger.getMsoLogger(MsoLogger.Catalog.BPEL, OofHomingV2.class);
    ...
    public void callOof(BuildingBlockExecution execution){
        ...
        try{
            ...
            log.trace("Completed Oof Homing Call Oof");
        }catch(BpmnError e){
    ...
    public void processSolution(BuildingBlockExecution execution, String asyncResponse){
        log.trace("Started Oof Homing Process Solution");
        try{
            ...
            log.debug("Processing Oof asyncronous response");
            ...
            log.trace("Completed Oof Homing Process Solution");
        }catch(BpmnError e){
            exceptionUtil.buildAndThrowWorkflowException(execution, Integer.parseInt(e.getErrorCode()), e.getMessage());
        }catch(BadResponseException e){
            exceptionUtil.buildAndThrowWorkflowException(execution, 400, e.getMessage());
