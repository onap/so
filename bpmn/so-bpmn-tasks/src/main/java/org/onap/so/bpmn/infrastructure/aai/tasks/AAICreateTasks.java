...
public class AAICreateTasks {
	 ...
	 *
	 * It will check if the Vnf Name is exits in A&AI then it will throw the
	...
	/**
	 * BPMN access method to execute NetworkCollectionInstanceGroup operation (PUT)
	 * in AAI
	 *
	 * @param execution
	 * @throws Exception
			...
			if (fqdns != null && !fqdns.isEmpty()) {
				...
				if (fqdnCount > 0) {
					for (int i = 0; i < fqdnCount; i++) {
						...
						Optional<org.onap.aai.domain.yang.NetworkPolicy> oNetPolicy = aaiNetworkResources
								.getNetworkPolicy(uri);
	...
	/**
	 * Groups existing vf modules by the model uuid of our new vf module and returns
	 * the lowest unused index
	 *
	 * if we have a module type A, and there are 3 instances of those, and then
	 * module type B has 2 instances, if we are adding a new module type A, the
	 * vf-module-index should be 3 assuming contiguous indices (not 5, or 2)
	 *
	 */
