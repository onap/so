...
public class EtsiVnfDeleteTask {
  ...
  /**
   * Create {@link checkVnfmFlag} object with required fields and store it in
   * {@link org.camunda.bpm.engine.delegate.DelegateExecution}
   * 
   * @param execution {@link org.onap.so.bpmn.common.DelegateExecutionImpl}
   */
  public void checkVnfmFlag(final BuildingBlockExecution execution) {
    try {
      LOGGER.debug("Executing checkVnfmFlag ...");

      final GenericVnf vnf = extractPojosForBB.extractByKey(execution, GENERIC_VNF_ID);

      if (vnf.getSelflink() != null) {
        LOGGER.info("VNFM Flag : {}", true);
        execution.setVariable(Constants.VNFM_FLAG_PARAM_NAME, true);
      }
      LOGGER.debug("Finished executing checkVnfmFlag ...");
    } catch (final Exception exception) {
      LOGGER.error("Unable to execute checkVnfmFlag", exception);
      exceptionUtil.buildAndThrowWorkflowException(execution, 1210, exception);
    }
  }
  ...
  public void invokeVnfmAdapter(final BuildingBlockExecution execution) {
    try {
      ...
      if (!response.isPresent()) {
        ...
        exceptionUtil.buildAndThrowWorkflowException(execution, 1212, errorMessage);
      }
      ...
      exceptionUtil.buildAndThrowWorkflowException(execution, 1213, exception);
    }
  }
}
