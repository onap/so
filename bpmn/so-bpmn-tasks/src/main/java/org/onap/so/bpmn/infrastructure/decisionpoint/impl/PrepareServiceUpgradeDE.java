...
/**
 * Fetches health check workflow based on the controller_scope
 * Invoke the corresponding health check workflow after validation
...
public class PrepareServiceUpgradeDE implements JavaDelegate {
    ...
    public void execute(DelegateExecution execution) throws Exception {
        ...
        if(wflToInvoke.equals(PNF_HC_WORKFLOW)) {
            ...
            if(isValid == true){
                ProcessInstanceWithVariables instance = runtimeService.createProcessInstanceByKey(wflToInvoke)
    ...
    /**
     * This method returns the health check workflow names based on the controller scope
     * At present the workflow names are hardcoded.
    ...
    private boolean validateParamsForPnf(DelegateExecution execution){
        ...
        List<String> pnfHealthCheckParams = Arrays.asList("pnfName", "pnf-id", "mode",
                                                        "pnf-ipv4-address", "service-instance-id");
