...
public class ConfigDeployVnf {
	...
	public void preProcessRequest(BuildingBlockExecution execution) {
		try {
			...
			payloadData.put("resolution-key", vnf.getVnfName());//
			payloadData.put("service-instance-id", serviceInstance.getServiceInstanceId());
			payloadData.put("service-model-uuid", "");//
			...
			String constructPayload = processingUtils.constructPayload(payloadData, "VNF", "ConfigDeploy");
	...
	/**
	 * Getting the vnf object and call AAI for configure Vnf
	 * 
	 * @param execution
	 */
	...
	/**
	 * Getting the vnf object and call AAI for configured Vnf
	 * 
	 * @param execution
	 */
