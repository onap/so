...
public class GenericPnfCDSProcessingDE implements ControllerRunnable<DelegateExecution> {
    ...
    @Autowired
    public GenericPnfCDSProcessingDE(ExceptionBuilder exceptionBuilder, AbstractCDSProcessingBBUtils cdsDispather) {
    ...
    public Boolean understand(ControllerContext<DelegateExecution> context) {
        ...
        return CDS_ACTOR.equalsIgnoreCase(context.getControllerActor()) && PNF_SCOPE.equalsIgnoreCase(scope)
                && !("config-assign".equalsIgnoreCase(context.getControllerAction())
                        || "config-deploy".equalsIgnoreCase(context.getControllerAction()));
    }
