...
public class WorkflowAction {
	...
	private static final String NAME_EXISTS_MULTIPLE = "(%s) and multiple combination of model-version-id + service-type + global-customer-id";
	private static final String NAME_EXISTS_WITH_DIFF_COMBINATION = "(%s) and global-customer-id (%s), service-type (%s), model-version-id (%s)";
	private static final String NAME_EXISTS_WITH_DIFF_CUSTOMIZATION_ID = "(%s), same parent and different customization id (%s)";
	...
	/**
	 * This method is used for fetching all the list of the BuildingBlocks from the
	 * OrchestrationFlow.
	 *
	 * Based on the aLaCarte it will query NorthBoundRequestCatalogDb .
	 *
	 * If the aLaCarte is false based on the requestAction & resourceType it will
	 * query again to NorthBoundRequestCatalogDb for BuildingBlocks.
	 *
	...
	public void selectExecutionList(DelegateExecution execution) throws Exception {
		...
		final String serviceType = Optional.ofNullable((String) execution.getVariable(BBConstants.G_SERVICE_TYPE))
				.orElse("");

		List<OrchestrationFlow> orchFlows = (List<OrchestrationFlow>) execution
				.getVariable(BBConstants.G_ORCHESTRATION_FLOW);
			...
			if (isRequestMacroServiceResume(aLaCarte, resourceType, requestAction, serviceInstanceId)) {
				...
				if (aLaCarte) {
					...
					if (resourceType == WorkflowType.SERVICE && requestAction.equalsIgnoreCase(ASSIGNINSTANCE)) {
						...
						if (containsService) {
							foundRelated = traverseUserParamsService(execution, resourceCounter, sIRequest,
									requestAction);
						}
						if (!foundRelated) {
	...
	protected <T> List<T> getRelatedResourcesInVnfc(Vnfc vnfc, Class<T> resultClass, AAIObjectType type) {
		...
		if (!relationshipsOp.isPresent()) {
			...
			Relationships relationships = relationshipsOp.get();
			List<AAIResultWrapper> configurationResultWrappers = this.getResultWrappersFromRelationships(relationships,
					type);
	...
	protected List<ExecuteBuildingBlock> getConfigBuildingBlocks(ConfigBuildingBlocksDataObject dataObj) {
		...
		String vfModuleCustomizationUUID = bbInputSetupUtils.getAAIVfModule(vnfId, vfModuleId)
		...
		for (org.onap.aai.domain.yang.Vnfc vnfc : vnfcs) {
			...
			if (configurations.size() > 1) {
				String multipleRelationshipsError = "Multiple relationships exist from VNFC " + vnfc.getVnfcName()
						+ " to Configurations";
	...
	private void updateResourceIdsFromAAITraversal(List<ExecuteBuildingBlock> flowsToExecute,
		...
		Arrays.stream(WorkflowType.values()).filter(type -> !type.equals(WorkflowType.SERVICE)).forEach(type -> {
			List<Resource> resources = resourceCounter.stream().filter(x -> type.equals(x.getResourceType()))
					.collect(Collectors.toList());
	...
	protected void traverseCatalogDbService(DelegateExecution execution, ServiceInstancesRequest sIRequest,
		...
		if (service == null) {
			...
			RelatedInstance relatedVpnBinding = bbInputSetupUtils
					.getRelatedInstanceByType(sIRequest.getRequestDetails(), ModelType.vpnBinding);
			RelatedInstance relatedLocalNetwork = bbInputSetupUtils
					.getRelatedInstanceByType(sIRequest.getRequestDetails(), ModelType.network);
	...
	protected String getExistingAAIVrfConfiguration(RelatedInstance relatedVpnBinding,
		...
		if (relationshipsOp.isPresent()) {
			List<AAIResultWrapper> configurationsRelatedToLocalNetwork = relationshipsOp.get()
					.getByType(AAIObjectType.CONFIGURATION);
	...
	protected boolean vrfConfigurationAlreadyExists(RelatedInstance relatedVpnBinding, Configuration vrfConfiguration,
		...
		if (vrfConfiguration.getConfigurationType().equalsIgnoreCase("VRF-ENTRY")) {
			...
			if (relationshipsConfigOp.isPresent()) {
				Optional<VpnBinding> relatedInfraVpnBindingOp = workflowActionUtils
						.extractRelationshipsVpnBinding(relationshipsConfigOp.get());
	...
	protected void traverseNetworkCollection(DelegateExecution execution, List<Resource> resourceCounter,
		...
		if (service.getVnfCustomizations() == null || service.getVnfCustomizations().isEmpty()) {
			...
			if (customizations.isEmpty()) {
				...
				CollectionResourceCustomization collectionResourceCustomization = findCatalogNetworkCollection(
						execution, service);
					...
					if (collectionResourceCustomization.getCollectionResource() != null) {
						if (collectionResourceCustomization.getCollectionResource().getInstanceGroup() != null) {
							...
							if (toscaNodeType != null && toscaNodeType.contains("NetworkCollection")) {
								int minNetworks = 0;
								org.onap.so.db.catalog.beans.InstanceGroup instanceGroup = collectionResourceCustomization
										.getCollectionResource().getInstanceGroup();
		...
		try {
			...
			if (serviceInstanceMSO.getConfigurations() != null) {
				for (Configuration config : serviceInstanceMSO.getConfigurations()) {
					Optional<org.onap.aai.domain.yang.Configuration> aaiConfig = aaiConfigurationResources
							.getConfiguration(config.getConfigurationId());
	...
	private void findConfigurationsInsideVfModule(DelegateExecution execution, String vnfId, String vfModuleId,
		...
		try {
			...
			if (relationshipsOp.isPresent()) {
				...
				if (relationshipsOp.isPresent()) {
					Optional<Configuration> config = workflowActionUtils
							.extractRelationshipsConfiguration(relationshipsOp.get());
					if (config.isPresent()) {
						aaiResourceIds.add(new Pair<WorkflowType, String>(WorkflowType.CONFIGURATION,
									...
									if (vfModuleCustomization != null) {
										...
										if (vfModuleCustomization.getVfModule() != null
											...
											if (vfModule.getModelInfo() != null
												...
												vfModuleCustomizationUUID = vfModule.getModelInfo()
			...
			if ("SERVICE".equalsIgnoreCase(type.toString())) {
				...
				if (instanceName != null) {
					...
					if (serviceInstanceAAI.isPresent()) {
						...
						ServiceInstances aaiServiceInstances = bbInputSetupUtils
								.getAAIServiceInstancesGloballyByName(instanceName);
						if (aaiServiceInstances != null) {
							if (aaiServiceInstances.getServiceInstance() != null
								...
								if (aaiServiceInstances.getServiceInstance().size() > 1) {
									...
									ServiceInstance si = aaiServiceInstances.getServiceInstance().stream().findFirst()
											.get();
									Map<String, String> keys = bbInputSetupUtils
											.getURIKeysFromServiceInstance(si.getServiceInstanceId());
				...
				if (vnf != null && vnf.getVfModules() != null) {
					for (org.onap.aai.domain.yang.VfModule vfModule : vnf.getVfModules().getVfModule()) {
						Optional<VolumeGroup> volumeGroupFromVfModule = bbInputSetupUtils
								.getRelatedVolumeGroupByNameFromVfModule(vnf.getVnfId(), vfModule.getVfModuleId(),
										instanceName);
				...
				Optional<org.onap.aai.domain.yang.Configuration> configuration = bbInputSetupUtils
						.getRelatedConfigurationByNameFromServiceInstance(workflowResourceIds.getServiceInstanceId(),
								instanceName);
