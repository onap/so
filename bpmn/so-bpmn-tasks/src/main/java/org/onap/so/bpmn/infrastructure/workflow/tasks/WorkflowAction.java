...
public class WorkflowAction {
                ...
                if (aLaCarte) {
                    ...
                    flowsToExecute =
                            buildExecuteBuildingBlockList(orchFlows, resourceCounter, requestId, apiVersion, resourceId,
                                    ...
                                    .stream().map(ExecuteBuildingBlock.Builder::build).collect(Collectors.toList());
            ...
            for (org.onap.aai.domain.yang.Configuration configuration : configurations) {
                ...
                for (OrchestrationFlow orchFlow : result) {
                    ...
                    ExecuteBuildingBlock.Builder ebb = buildExecuteBuildingBlock(orchFlow, dataObj.getRequestId(),
                            ...
                            dataObj.getWorkflowResourceIds(), dataObj.getRequestDetails(), false, null,
                            vnfcName, true);
    ...
    protected ExecuteBuildingBlock.Builder buildExecuteBuildingBlock(OrchestrationFlow orchFlow, String requestId,
        ...
        BuildingBlock.Builder buildingBlock = new BuildingBlock.Builder().withBpmnFlowName(orchFlow.getFlowName())
                .withMsoId(UUID.randomUUID().toString()).withIsVirtualLink(isVirtualLink).withVirtualLinkKey(virtualLinkKey)
                .withKey(Optional.ofNullable(resource).map(Resource::getResourceId).orElse(""));

        ExecuteBuildingBlock.Builder executeBuildingBlock = new ExecuteBuildingBlock.Builder().withApiVersion(apiVersion)
        .withaLaCarte(aLaCarte).withRequestAction(requestAction).withResourceId(resourceId).withVnfType(vnfType)
        .withWorkflowResourceIds(workflowResourceIds).withRequestId(requestId).withBuildingBlock(buildingBlock.build())
