        ...
        try {
            ...
            if (isRequestMacroServiceResume(isALaCarte, resourceType, requestAction, serviceInstanceId)) {
                ...
                if (isALaCarte) {
                    ...
                    UserParamsServiceTraversal userParamsServiceTraversal = new UserParamsServiceTraversal(catalogDbClient, exceptionBuilder);
                    if (resourceType == WorkflowType.SERVICE && requestAction.equalsIgnoreCase(ASSIGNINSTANCE)) {
                        ...
                        if (getListOfUserParams(sIRequest) != null) {
                            ...
                            if (containsService) {
                               resourceList = userParamsServiceTraversal.getResourceListFromUserParams(execution, getListOfUserParams(sIRequest), serviceInstanceId, requestAction);
                            }
                        } else {
                        ...
                        if (containsService) {
                            resourceList = userParamsServiceTraversal.getResourceListFromUserParams(
                                    execution, getListOfUserParams(sIRequest), serviceInstanceId, requestAction);
                        }
                        if (!containsWorkflowType(resourceList, WorkflowType.VNF)
