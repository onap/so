...
public class WorkflowActionBBTasks {
	 ...
	 *
	 * It will get the currentSequence from execution object and increase its count
	 * by +1 and check if the currentSequence is grater than or equal to
	 * flowsToExecute size then set the currentSequence to the execution object to
	 * execute.
	 *
	 * @param execution
	 * @throws @return
	 ...
	 *
	 * It will get the currentSequence from execution and check if the
	 * currentSequence is grater than 1 then it will update the requestDbclient.
	 *
	 * @param execution
	 * @throws @return
	 */
	 ...
	 *
	 * It will update the requestDB according the macroAction if it is alacarte or
	 * macro.
	 *
	 * @param execution
	 * @throws @return
	...
	/**
	 * Rollback will only handle Create/Activate/Assign Macro flows. Execute layer
	 * will rollback the flow its currently working on.
	 */
	 ...
	 *
	 * It will check if the aLaCarte is success then it will execute the BB and
	 * activate the Vfmodule.
	 *
	 * @param execution
	 * @throws @return
	 */
	...
	protected void postProcessingExecuteBBActivateVfModule(DelegateExecution execution, ExecuteBuildingBlock ebb,
		...
		try {
			...
			String serviceModelUUID = bbInputSetupUtils.getAAIServiceInstanceById(serviceInstanceId)
					.getModelVersionId();
			...
			List<Vnfc> vnfcs = workflowAction.getRelatedResourcesInVfModule(vnfId, vfModuleId, Vnfc.class,
					AAIObjectType.VNFC);
