    ...
    public void run(List<ExecuteBuildingBlock> flowsToExecute, ExecuteBuildingBlock currentBB,
        ...
        if (currentBB.getBuildingBlock().getBpmnScope().equalsIgnoreCase("VNF")
            ...
            if (CollectionUtils.isEmpty(vnfResourceCustomizations)) {
                ...
                currentSequencySkipCheck(execution, skipConfigVNF);
            }

        } else if (Strings.isNotEmpty(customizationUUID)
                && currentBB.getBuildingBlock().getBpmnScope().equalsIgnoreCase("VFModule")
                && (currentBB.getBuildingBlock().getBpmnAction().equalsIgnoreCase("vfModuleConfigAssign")
                        || currentBB.getBuildingBlock().getBpmnAction().equalsIgnoreCase("vfModuleConfigDeploy"))) {
            ...
            currentSequencySkipCheck(execution, skipVfModule);
        }

    }

    private void currentSequencySkipCheck(BuildingBlockExecution execution, boolean skipModule) {
        if (skipModule) {
