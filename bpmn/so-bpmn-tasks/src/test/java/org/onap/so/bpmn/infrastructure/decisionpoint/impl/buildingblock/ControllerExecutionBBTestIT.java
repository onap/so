...
public class ControllerExecutionBBTestIT extends BaseIntegrationTest {
    ...
    @Test
    public void testExecution_appcHealthCheck_actionExecuted() {
        configureAppcHealthCheck();

        controllerExecutionBB.execute(execution);

        verify(appCClient, times(1)).runAppCCommand(eq(Action.HealthCheck), eq(TEST_MSO_REQUEST_ID),
                eq(genericVnf.getVnfId()), eq(null), any(HashMap.class), eq("appc"));
    }

    @Test
    public void testExecution_appcUpgradeHealthCheck_actionExecuted() {
        configureAppcUpgrade();
        controllerExecutionBB.execute(execution);

        verify(appCClient, times(1)).runAppCCommand(eq(Action.UpgradePreCheck), eq(TEST_MSO_REQUEST_ID), eq(null),
                any(Optional.class), any(HashMap.class), eq("appc"));
    }

    private void configureAppcUpgrade() {
        execution.setVariable("actor", "appc");
    ...
    private void configureAppcHealthCheck() {
        execution.setVariable("actor", "appc");
        execution.setVariable("scope", "vnf");
        execution.setVariable("action", TEST_APPC_ACTION);
        ...
        controllerSelectionReference.setControllerName("appc");
        when(catalogDbClient.getControllerSelectionReferenceByVnfTypeAndActionCategory(anyString(), anyString()))
