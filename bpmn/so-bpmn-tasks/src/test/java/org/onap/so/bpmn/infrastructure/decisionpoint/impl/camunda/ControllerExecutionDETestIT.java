...
public class ControllerExecutionDETestIT extends BaseIntegrationTest {
    ...
    public void testExecution_appcHealthCheck_actionExecuted() {
        ...
        verify(appCClient, times(1)).runAppCCommand(eq(Action.HealthCheck), eq(TEST_MSO_REQUEST_ID),
                eq(genericVnf.getVnfId()), any(Optional.class), any(HashMap.class), eq("appc"));
    }

    private void configureAppcHealthCheck() {
        delegateExecution.setVariable("actor", "appc");
        delegateExecution.setVariable("action", TEST_APPC_ACTION);

        setServiceInstance();
        genericVnf = setGenericVnf();

        delegateExecution.setVariable("vnfId", genericVnf.getVnfId());
        delegateExecution.setVariable("vnfName", genericVnf.getVnfName());
        delegateExecution.setVariable("vnfHostIpAddress", genericVnf.getIpv4OamAddress());
        delegateExecution.setVariable("controllerType", "appc");

        delegateExecution.setVariable("healthCheckIndex", 1);
    }
