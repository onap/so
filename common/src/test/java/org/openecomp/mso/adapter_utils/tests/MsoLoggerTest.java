...
public class MsoLoggerTest {
     ...
     */
	@Test
    public final void testGetSeverityLevel () throws IllegalAccessException, NoSuchMethodException, InvocationTargetException {
     ...
     */
	@Test
    public final void testGetFinalServiceName () throws IllegalAccessException, NoSuchMethodException, InvocationTargetException {
	...
	public final void testPrepareMsg()
		...
		Assert.assertTrue(
			MDC.get(MsoLogger.REQUEST_ID).equals("trace-#") && MDC.get(MsoLogger.SERVICE_INSTANCE_ID).equals("trace-#")
		...
		Assert.assertTrue(MDC.get(MsoLogger.REQUEST_ID).equals("testReqId") && MDC.get(MsoLogger.SERVICE_INSTANCE_ID)
			.equals("testSvcId") && MDC.get(MsoLogger.SERVICE_NAME).equals("testServiceName3")
			&& MDC.get(MsoLogger.TIMER) == null && MDC.get(MsoLogger.ALERT_SEVERITY).equals("2"));

		MsoLogger.setServiceName("testServiceName2");
		invokePrepareMsg("WARN", msgNull, msgNull);
		Assert.assertTrue(MDC.get(MsoLogger.REQUEST_ID).equals("testReqId") && MDC.get(MsoLogger.SERVICE_INSTANCE_ID)
			.equals("testSvcId") && MDC.get(MsoLogger.SERVICE_NAME).equals("testServiceName2")
		...
		Assert.assertTrue(
			MDC.get(MsoLogger.REQUEST_ID).equals("reqId2") && MDC.get(MsoLogger.SERVICE_INSTANCE_ID).equals("servId2")
     ...
     */
	@Test
	public final void testLogMethods() throws IOException {
		MDC.clear();
		...
		msoLogger.warn(MessageEnum.GENERAL_WARNING, "warning test", "", "", MsoLogger.ErrorCode.UnknownError,
			"warning test");
		msoLogger
			.error(MessageEnum.GENERAL_EXCEPTION, "target entity", "target service", MsoLogger.ErrorCode.UnknownError,
				"error test");
		...
		Assert.assertTrue(
			stringArray[size - 3].contains("|reqId2|main|MSO.testServiceName||target entity|target service|INFO|null||")
				&& stringArray[size - 3]
				.contains("||MSO-GENERAL-5408I Successfully update Logger: testLogger from level INFO to level DEBUG"));
		Assert.assertTrue(
			stringArray[size - 2].contains("|reqId2|main|MSO.testServiceName||||WARN|UnknownError|warning test|")
				&& stringArray[size - 2].contains("|MSO-GENERAL-5401W WARNING: warning test"));
		Assert.assertTrue(stringArray[size - 1]
			.contains("|reqId2|main|MSO.testServiceName||target entity|target service|ERROR|UnknownError|error test|")
	...
	public final void testRecordMetricEvent() throws IOException {
		...
		Assert.assertTrue(stringArray[0].contains(
			"|reqId|servId|main||testRecordMetricEvent||VNF|createVNF|COMPLETE|0|Successful|Test UUID as JBoss not found|INFO|0|"));
		// count the occurance of symbol "|"
		Assert.assertTrue((stringArray[0].length() - stringArray[0].replace("|", "").length()) == 28);
		Assert.assertTrue(stringArray[1].contains(
			"|reqId|servId|main||testRecordMetricEvent|testUser|SDNC|removeSDNC|ERROR|501|Exception|Test UUID as JBoss not found|INFO|0|")
     ...
     */
	@Test
	public final void testRecordAuditEvent() throws IOException {
		MDC.clear();
		MsoLogger.setLogContext("reqId", "servId");
		msoLogger.recordAuditEvent(123456789L, MsoLogger.StatusCode.COMPLETE, MsoLogger.ResponseCode.Suc, "Successful");
		MDC.put(MsoLogger.REMOTE_HOST, "127.0.0.1");
		MDC.put(MsoLogger.PARTNERNAME, "testUser");
		msoLogger.recordAuditEvent(123456789L, MsoLogger.StatusCode.ERROR, MsoLogger.ResponseCode.ServiceNotAvailable,
			"Exception");
		...
		Assert.assertTrue(stringArray[0].contains(
			"|reqId|servId|main||testRecordAuditEvent||COMPLETE|0|Successful|Test UUID as JBoss not found|INFO|0|"));
		// count the occurance of symbol "|"
		Assert.assertTrue((stringArray[0].length() - stringArray[0].replace("|", "").length()) == 25);
		Assert.assertTrue(stringArray[1].contains(
			"|reqId|servId|main||testRecordAuditEvent|testUser|ERROR|501|Exception|Test UUID as JBoss not found|INFO|0|")
	...
	private static String invokePrepareMsg(String arg1, String arg2, String arg3)
		...
		Method method = MsoLogger.class.getDeclaredMethod("prepareMsg", String.class, String.class, String.class);
		method.setAccessible(true);
		return method.invoke(msoLogger, arg1, arg2, arg3).toString();
	}

    // User reflection to invoke to avoid change the publicity of the method
	private static Object invokePriveMethod(String methodName, String arg)
		throws InvocationTargetException, IllegalAccessException, NoSuchMethodException {
		Method method = MsoLogger.class.getDeclaredMethod(methodName, String.class);
		method.setAccessible(true);
