...
import org.onap.so.aaiclient.api.entities.AAIVersion;
...
public class AAIResourcesClientTest {
    ...
    public AAIResourcesClient aaiClient;
    ...
    public void setup() {
        ...
        RestTemplate restTemplate =
                new RestTemplateBuilder().defaultHeader(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON_VALUE)
                        .rootUri("http://localhost:" + wireMockServer.port() + "/aai/" + AAIVersion.LATEST).build();
        aaiClient = new AAIResourcesClient(restTemplate);
    }
    ...
    public void verifyGet() {
        // AAIResourceUri path = AAIUriFactory.createResourceUri(AAIFluentTypeBuilder.network().pserver("test"));
        String path = "/cloud-infrastructure/pservers/pserver/pserver-hostname-test"; // TODO: Replace with
                                                                                      // builder

        WireMock.stubFor(get(urlPathEqualTo("/aai/" + AAIVersion.LATEST + path.toString()))
        ...
        AAIResultWrapper<Pserver> result = client.get(path, Pserver.class);
