...
public class AAIRestClientImpl implements AAIRestClientI {
    ...
    @Override
    public Optional<ServiceInstance> getServiceInstanceById(String globalSubscriberId, String serviceType,  String serviceInstanceId) {
        Response response = new AAIResourcesClient()
                .getFullResponse(AAIUriFactory.createResourceUri(AAIObjectType.SERVICE_INSTANCE,
                        globalSubscriberId,
                        serviceType,
                        serviceInstanceId));
        if (response.getStatus() != 200) {
