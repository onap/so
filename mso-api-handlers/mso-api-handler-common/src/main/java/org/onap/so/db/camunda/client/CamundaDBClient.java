...
public class CamundaDBClient {
    ...
    String sqlQuery = "SELECT start.ACT_ID_ AS resume_from " + "FROM ACT_HI_ACTINST start "
    ...
    public String findResumeFromBB(String requestId) {
        String bbName = null;
        try {
            logger.trace("****** In Try Block to fetch resumeFrom ****** CamundaDBClient");
    ...
    public String findProcessInstanceId(String requestId) {
        ...
        try {
            logger.trace("****** In Try Block to fetch processInstanceId ****** CamundaDBClient");
    ...
    protected String stripStartPrefix(String resumeFrom) {
        ...
        if (resumeFrom != null && resumeFrom.startsWith("Start_")) {
            ...
            logger.info("****** Cleaned resumeFrom Building block: {} ******", bbName);
        }
        return bbName;
    }
}
