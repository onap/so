...
public class AbortOrchestrationRequest {
    ...
    private final PauseOrchestrationRequest pauseOrchestrationRequest;

    private final InfraActiveRequestsService infraActiveRequestsService;
    ...
    public Response abortOrchestrationRequest(@PathParam("version") String version,
        ...
        try {
            infraActiveRequestsList = infraActiveRequestsService.getListOfRequestsInProgress();
    ...
    protected Response abortRequest(List<InfraActiveRequests> infraActiveRequests,
        ...
        ServiceInstancesRequest serviceInstancesRequest = null;
        serviceInstancesRequest =
                requestHandlerUtils.convertJsonToServiceInstanceRequest(requestBody, action, requestId, requestUri);
        Boolean aLaCarte = serviceInstancesRequest.getRequestDetails().getRequestParameters().getALaCarte();

        String pnfCorrelationId = serviceInstances.getPnfCorrelationId(serviceInstancesRequest);
        RecipeLookupResult recipeLookupResult =
                requestHandlerUtils.getServiceInstanceOrchestrationURI(serviceInstancesRequest, action,
                        msoRequest.getAlacarteFlag(serviceInstancesRequest), currentActiveRequest);

        List<String> listOfProcessInstanceId = camundaDBClient.fetchListOfProcessInstanceId();
        if (listOfProcessInstanceId == null || listOfProcessInstanceId.isEmpty()) {
            throw new ValidateException.Builder("Process instance id is not saved yet please try again:",
                    HttpStatus.SC_NOT_FOUND, ErrorNumbers.SVC_BAD_PARAMETER).build();
        }
        RequestClientParameter requestClientParameter = pauseOrchestrationRequest.setRequestClientParameter(
                recipeLookupResult, version, currentActiveRequest, pnfCorrelationId, serviceInstancesRequest, aLaCarte);
