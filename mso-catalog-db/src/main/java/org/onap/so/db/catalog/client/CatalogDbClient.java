...
public class CatalogDbClient {
    ...
    private final WebClient networkRecipeClient;

    private final WebClient networkResourceCustomizationClient;

    private final WebClient vnfResourceClient;

    private final WebClient vnfResourceCustomizationClient;

    private final WebClient vnfRecipeClient;

    private final WebClient vfModuleCustomizationClient;

    private final WebClient vfModuleClient;

    private final WebClient vnfComponentsRecipeClient;

    private final WebClient orchestrationClient;

    private final WebClient northBoundRequestClient;

    private final WebClient rainyDayHandlerStatusClient;

    private final WebClient buildingBlockDetailClient;

    private final WebClient orchestrationStatusStateTransitionDirectiveClient;

    private final WebClient vnfcInstanceGroupCustomizationClient;

    private final WebClient collectionResourceInstanceGroupCustomizationClient;

    private final WebClient instanceGroupClient;

    private final WebClient networkCollectionResourceCustomizationClient;

    private final WebClient collectionNetworkResourceCustomizationClient;

    private final WebClient serviceRecipeClient;

    private final WebClient networkResourceClient;

    private final WebClient externalServiceToInternalServiceClient;

    private final WebClient cloudSiteClient;

    private final WebClient homingInstanceClient;

    private final WebClient cloudifyManagerClient;

    private final WebClient controllerSelectionReferenceClient;

    private final WebClient pnfResourceClient;

    private final WebClient pnfResourceCustomizationClient;

    private final WebClient workflowClient;

    private final WebClient bbNameSelectionReferenceClient;

    private final WebClient processingFlagsClient;

    private final WebClient buildingBlockRollbackClient;
    ...
    public CatalogDbClient() {
        ...
        serviceClient = webClient.mutate().build();
        networkRecipeClient = webClient.mutate().build();
        networkResourceCustomizationClient = webClient.mutate().build();
        vnfResourceClient = webClient.mutate().build();
        vnfResourceCustomizationClient = webClient.mutate().build();
        vnfRecipeClient = webClient.mutate().build();
        orchestrationClient = webClient.mutate().build();
        vfModuleCustomizationClient = webClient.mutate().build();
        vfModuleClient = webClient.mutate().build();
        vnfComponentsRecipeClient = webClient.mutate().build();
        northBoundRequestClient = webClient.mutate().build();
        rainyDayHandlerStatusClient = webClient.mutate().build();
        buildingBlockDetailClient = webClient.mutate().build();
        orchestrationStatusStateTransitionDirectiveClient = webClient.mutate().build();
        vnfcInstanceGroupCustomizationClient = webClient.mutate().build();
        collectionResourceInstanceGroupCustomizationClient = webClient.mutate().build();
        instanceGroupClient = webClient.mutate().build();
        networkCollectionResourceCustomizationClient = webClient.mutate().build();
        collectionNetworkResourceCustomizationClient = webClient.mutate().build();
        cloudSiteClient = webClient.mutate().build();
        homingInstanceClient = webClient.mutate().build();
        cloudifyManagerClient = webClient.mutate().build();
        serviceRecipeClient = webClient.mutate().build();
        controllerSelectionReferenceClient = webClient.mutate().build();
        externalServiceToInternalServiceClient = webClient.mutate().build();
        pnfResourceClient = webClient.mutate().build();
        pnfResourceCustomizationClient = webClient.mutate().build();
        workflowClient = webClient.mutate().build();
        bbNameSelectionReferenceClient = webClient.mutate().build();
        processingFlagsClient = webClient.mutate().build();
        networkResourceClient = webClient.mutate().build();
        buildingBlockRollbackClient = webClient.mutate().build();
