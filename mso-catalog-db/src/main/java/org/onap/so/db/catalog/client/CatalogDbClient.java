...
public class CatalogDbClient {
    ...
    private <T> List<T> getMultipleResources(Class<T> clazz, URI uri) {
        logger.debug("getMultipleResources called with uri = {}", uri);
        return webClient.get().uri(uri).retrieve().bodyToFlux(clazz)
                .onErrorResume(WebClientResponseException.class, notFound -> Mono.empty()).collectList().block();
    }
