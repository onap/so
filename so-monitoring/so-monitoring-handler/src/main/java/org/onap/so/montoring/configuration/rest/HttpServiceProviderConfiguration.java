...
import org.onap.so.montoring.configuration.ServiceType;
import org.onap.so.montoring.configuration.ServiceType.Type;
...
public class HttpServiceProviderConfiguration {
    ...
    @Bean
    @ServiceType(Type.CAMUNDA)
    public HttpRestServiceProvider camundaHttpRestServiceProvider(@Autowired final RestTemplate restTemplate,
            @ServiceType(Type.CAMUNDA) @Autowired final AuthenticationDetails authenticationDetails) {
        return httpRestServiceProvider(restTemplate, authenticationDetails);
    }

    @Bean
    @ServiceType(Type.DATABASE)
    public HttpRestServiceProvider databaseHttpRestServiceProvider(@Autowired final RestTemplate restTemplate,
            @ServiceType(Type.DATABASE) @Autowired final AuthenticationDetails authenticationDetails) {
        return httpRestServiceProvider(restTemplate, authenticationDetails);
    }

    @Bean
    public HttpRestServiceProvider httpRestServiceProvider(final RestTemplate restTemplate,
