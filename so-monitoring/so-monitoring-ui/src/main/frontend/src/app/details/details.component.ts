  ...
  async getProcessDefinition(procDefId) {
    await this.data.getProcessDefinition(procDefId).subscribe(
      async (data: PDI) => {
        ...
        console.log(data);
        await this.displayCamundaflow(this.processDefinition.processDefinitionXml);
      }, error => {
        console.log(error);
  ...
  async  getProcInstance(procInstId) {
    await this.data.getProcessInstance(procInstId).then(
      async (data: PII) => {
        ...
        console.log("Process definition id: " + this.processInstance.processDefinitionId);
        await this.getProcessDefinition(this.processInstance.processDefinitionId);
      }, error => {
  console.log(error);
  ...
  async ngOnInit() {
    ...
    this.route.params.subscribe(
      async params => {
        ...
        console.log('>>>>> id: ' + this.process$);
        this.procInstIdValue = this.process$ as string;
        console.log("Will GET Process instanc using id: " + this.procInstIdValue);
        await this.getProcInstance(this.procInstIdValue);
        this.getActInst(this.procInstIdValue);
        this.getVarInst(this.procInstIdValue);
      });
