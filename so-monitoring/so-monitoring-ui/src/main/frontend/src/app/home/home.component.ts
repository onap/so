...
import { MatFormFieldModule, MatInputModule, MatPaginator, MatSort, MatTableDataSource, PageEvent } from '@angular/material';
...
export class HomeComponent implements OnInit {
  ...
  makeCall() {
    ...
    var search = this.searchData.getSearchRequest().subscribe((result: SearchRequest) => {
      ...
      this.dataServ.retrieveInstance(result.getFilters(), result.getStartTimeInMilliseconds(), result.getEndTimeInMilliseconds())
        .subscribe((dataServ: Process[]) => {
          this.spinner.hide();
          var processData: Process[] = dataServ;
          ...
          this.popup.info("Number of records found: " + dataServ.length)
  ...
  getProcessIsntanceId(requestId: string) {
    ...
    var response = this.dataServ.getProcessInstanceId(requestId).subscribe((data) => {
      if (data.status == 200) {
        this.spinner.hide();
